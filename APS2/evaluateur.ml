...
and pi prim mem ctx = 
	match prim     with 
		|Prim("add", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in let right = (eval_expr e2 mem ctx) in (toN left) + (toN right)
		|Prim("sub", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in let right = (eval_expr e2 mem ctx) in (toN left) - (toN right)
		|Prim("div", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in let right = (eval_expr e2 mem ctx) in (toN left) / (toN right)
		|Prim("mul", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in let right = (eval_expr e2 mem ctx) in (toN left) * (toN right)
		|Prim("and", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in if ((toN left) == 0) then 0 else let right = (eval_expr e2 mem ctx) in (toN right)
		|Prim("or", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in if ((toN left) == 1) then 1 else let right = (eval_expr e2 mem ctx) in (toN right)
		|Prim("eq", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in let right = (eval_expr e2 mem ctx) in if ((toN left) == (toN right)) then 1 else 0
		|Prim("lt", e1::e2::[]) -> let left = (eval_expr e1 mem ctx) in let right = (eval_expr e2 mem ctx) in if ((toN left) < (toN right)) then 1 else 0

		|Prim("not", e1::[]) -> let left = (eval_expr e1 mem ctx) in ((toN left) + 1) mod 2
